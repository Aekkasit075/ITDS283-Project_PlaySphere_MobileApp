...
class _ManageAppState extends State<ManageApp>
  ...
  Future<void> _loadDownloadedGames() async {
    ...
    List<String> gamesStringList =
        prefs.getStringList('downloaded_games') ?? [];

    setState(() {
      downloadedGames =
          gamesStringList.map((gameStr) {
            return Map<String, dynamic>.from(jsonDecode(gameStr));
          }).toList();
    });
  }
  ...
  Future<void> _deleteGame(int gameId) async {
    ...
    if (confirmDelete == true) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          // Step 2: Delete the comments for this game
          SharedPreferences prefs = await SharedPreferences.getInstance();
          prefs.remove(
            gameId.toString(),
          ); // Remove comments from SharedPreferences based on gameId

          // Step 3: Show success dialog for game deletion
          showDialog(
            context: context,
            builder: (BuildContext context) {
              return AlertDialog(
          ...
          // Step 4: Update the game data after deletion
          setState(() {
...
class GameCard extends StatelessWidget {
  ...
  const GameCard({Key? key, required this.app}) : super(key: key);
      ...
      child: Column(
        children: [
          ...
          ElevatedButton(
            ...
            child: Text(
              app["price"] == "Play" ? "Play" : (app["price"] ?? "N/A"),
              style: TextStyle(color: Colors.white),
            ),
            style: ElevatedButton.styleFrom(backgroundColor: Colors.blue),
          ),
